...
} from "@vaadin/react-components/Dialog.js";
...
export default function AddTagDialog({ open, setOpened }: AddTagDialogProps) {
    ...
    onSubmit: async (value: FormikTag, { setSubmitting, setErrors }) => {
      try {
        ...
        if (error instanceof EndpointError) {
          Notification.show((error as EndpointError).message, {
            theme: "error",
          });
        } else {
          Notification.show("Adding tag was not successful", {
            theme: "error",
          });
        }
      } finally {
  ...
  return (
    <Dialog
      ...
      header={<h3 className={"m-0"}>New Tag</h3>}
      footer={
